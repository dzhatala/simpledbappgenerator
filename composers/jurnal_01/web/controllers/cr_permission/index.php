...
$app->match('/cr_permission/list', function (Symfony\Component\HttpFoundation\Request $request) use ($app) {  
    $start = 0;
	...
	/**Creating enabler .. for  cr_permission:user_login_id  **/
	...
	/**Creating enabler .. for  cr_permission:crud_table_id  **/
    ...
    foreach($table_columns as $col){
		...
		$whereClause =  $whereClause . "   cr_permission.".$col . " LIKE '%". $searchValue ."%' ".$transform_text_to_key;
        
		//non external search version ...
		//$whereClause =  $whereClause . "   cr_permission.".$col . " LIKE '%". $searchValue ."%' ";
    ...
    $recordsTotal = $app['db']->executeQuery("SELECT * FROM `cr_permission`" . $whereClause . $orderClause)->rowCount();
    
    $find_sql = "SELECT * FROM `cr_permission`". $whereClause . $orderClause . " LIMIT ". $index . "," . $rowsPerPage;
    ...
    foreach($rows_sql as $row_key => $row_sql){
        for($i = 0; $i < count($table_columns); $i++){
			...
			else if($table_columns[$i] == 'crud_table_id'){
...
/* Download blob img */
$app->match('/cr_permission/download', function (Symfony\Component\HttpFoundation\Request $request) use ($app) { 
    ...
    $find_sql = "SELECT " . $fieldname . " FROM " . cr_permission . " WHERE ".$idfldname." = ?";
...
$app->match('/cr_permission', function () use ($app) {
	...
	foreach ($table_columns as $col){
		//var_dump($col) ;die;
		//array_push($tr_table_columns,$app['translator']->trans("cr_permission".$col));
		array_push($tr_table_columns,$col);
	}
	$table_columns=$tr_table_columns;
	/****/

    $primary_key = "cr_permission_id";	

    return $app['twig']->render('cr_permission/list.html.twig', array(
    	"table_columns" => $table_columns,
        "primary_key" => $primary_key
    ));
        
})
->bind('cr_permission_list');



$app->match('/cr_permission/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `cr_permission` (`user_login_id`, `crud_table_id`, `policy`) VALUES (?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['user_login_id'], $data['crud_table_id'], $data['policy']));            


            $app['session']->getFlashBag()->add(
                'success',
                array(
                    'message' => 'cr_permission created!',
                )
            );
            return $app->redirect($app['url_generator']->generate('cr_permission_list'));

        }
    }

    return $app['twig']->render('cr_permission/create.html.twig', array(
        "form" => $form->createView()
    ));
        
})
->bind('cr_permission_create');



$app->match('/cr_permission/edit/{id}', function ($id) use ($app) {

    $find_sql = "SELECT * FROM `cr_permission` WHERE `cr_permission_id` = ?";
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `cr_permission` SET `user_login_id` = ?, `crud_table_id` = ?, `policy` = ? WHERE `cr_permission_id` = ?";
            $app['db']->executeUpdate($update_query, array($data['user_login_id'], $data['crud_table_id'], $data['policy'], $id));            

	
            $app['session']->getFlashBag()->add(
                'success',
                array(
                    'message' => 'cr_permission edited!',
                )
            );
            return $app->redirect($app['url_generator']->generate('cr_permission_edit', array("id" => $id)));

        }
    }

    return $app['twig']->render('cr_permission/edit.html.twig', array(
        "form" => $form->createView(),
        "id" => $id
    ));
        
})
->bind('cr_permission_edit');



$app->match('/cr_permission/delete/{id}', function ($id) use ($app) {

    $find_sql = "SELECT * FROM `cr_permission` WHERE `cr_permission_id` = ?";
    $row_sql = $app['db']->fetchAssoc($find_sql, array($id));

    if($row_sql){
        $delete_query = "DELETE FROM `cr_permission` WHERE `cr_permission_id` = ?";
        $app['db']->executeUpdate($delete_query, array($id));

        $app['session']->getFlashBag()->add(
            'success',
            array(
                'message' => 'cr_permission deleted!',
            )
        );
    }
    else{
