...
$app->match('/ur_permission/list', function (Symfony\Component\HttpFoundation\Request $request) use ($app) {  
    $start = 0;
	...
	/**Creating enabler .. for  ur_permission:user_login_id  **/
	...
	/**Creating enabler .. for  ur_permission:crud_table_id  **/
    ...
    foreach($table_columns as $col){
		...
		$whereClause =  $whereClause . "   ur_permission.".$col . " LIKE '%". $searchValue ."%' ".$transform_text_to_key;
        
		//non external search version ...
		//$whereClause =  $whereClause . "   ur_permission.".$col . " LIKE '%". $searchValue ."%' ";
    ...
    $recordsTotal = $app['db']->executeQuery("SELECT * FROM `ur_permission`" . $whereClause . $orderClause)->rowCount();
    
    $find_sql = "SELECT * FROM `ur_permission`". $whereClause . $orderClause . " LIMIT ". $index . "," . $rowsPerPage;
...
/* Download blob img */
$app->match('/ur_permission/download', function (Symfony\Component\HttpFoundation\Request $request) use ($app) { 
    ...
    $find_sql = "SELECT " . $fieldname . " FROM " . ur_permission . " WHERE ".$idfldname." = ?";
...
$app->match('/ur_permission', function () use ($app) {
	...
	foreach ($table_columns as $col){
		//var_dump($col) ;die;
		//array_push($tr_table_columns,$app['translator']->trans("ur_permission".$col));
		array_push($tr_table_columns,$col);
	}
	$table_columns=$tr_table_columns;
	/****/

    $primary_key = "ur_permission_id";	

    return $app['twig']->render('ur_permission/list.html.twig', array(
    	"table_columns" => $table_columns,
        "primary_key" => $primary_key
    ));
        
})
->bind('ur_permission_list');



$app->match('/ur_permission/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['session']->getFlashBag()->add(
                'success',
                array(
                    'message' => 'ur_permission created!',
                )
            );
            return $app->redirect($app['url_generator']->generate('ur_permission_list'));

        }
    }

    return $app['twig']->render('ur_permission/create.html.twig', array(
        "form" => $form->createView()
    ));
        
})
->bind('ur_permission_create');



$app->match('/ur_permission/edit/{id}', function ($id) use ($app) {

    $find_sql = "SELECT * FROM `ur_permission` WHERE `ur_permission_id` = ?";
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['session']->getFlashBag()->add(
                'success',
                array(
                    'message' => 'ur_permission edited!',
                )
            );
            return $app->redirect($app['url_generator']->generate('ur_permission_edit', array("id" => $id)));

        }
    }

    return $app['twig']->render('ur_permission/edit.html.twig', array(
        "form" => $form->createView(),
        "id" => $id
    ));
        
})
->bind('ur_permission_edit');



$app->match('/ur_permission/delete/{id}', function ($id) use ($app) {

    $find_sql = "SELECT * FROM `ur_permission` WHERE `ur_permission_id` = ?";
    $row_sql = $app['db']->fetchAssoc($find_sql, array($id));

    if($row_sql){
        $delete_query = "DELETE FROM `ur_permission` WHERE `ur_permission_id` = ?";
        $app['db']->executeUpdate($delete_query, array($id));

        $app['session']->getFlashBag()->add(
            'success',
            array(
                'message' => 'ur_permission deleted!',
            )
        );
    }
    else{
