...
$app->match('/dr_permission/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `dr_permission` (`crud_table_id`, `user_login_id`, `record_id`, `policy`) VALUES (?, ?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['crud_table_id'], $data['user_login_id'], $data['record_id'], $data['policy']));            
...
$app->match('/dr_permission/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `dr_permission` SET `crud_table_id` = ?, `user_login_id` = ?, `record_id` = ?, `policy` = ? WHERE `dr_permission_id` = ?";
            $app['db']->executeUpdate($update_query, array($data['crud_table_id'], $data['user_login_id'], $data['record_id'], $data['policy'], $id));            
